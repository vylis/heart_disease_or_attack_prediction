...
def build_nn(hp=None):
    ...
    units = 32 if hp is None else hp.Int("units", min_value=32, max_value=512, step=32)
...
def cross_validation(X_train, y_train, X_val, y_val):
    ...
    for i, model in enumerate(models):
        if model == nn:
            ...
            # confusion matrix
            cm = confusion_matrix(y_vals, np.round(nn_preds))
            print(f"Neural Network Confusion Matrix: \n{cm}")

            # roc curve
            lb = LabelBinarizer()
            lb.fit(y_vals)
            y_vals_lb = lb.transform(y_vals)
            y_pred_lb = lb.transform(np.round(nn_preds))
            fpr, tpr, _ = roc_curve(y_vals_lb, y_pred_lb)
            roc_auc = auc(fpr, tpr)
            ...
            # measure performance with k-fold cross-validation
            scores = cross_val_score(model, X_val, y_val, cv=kfold, scoring="accuracy")
            ...
            y_pred_lb = lb.transform(model.predict(X_val))
            fpr, tpr, _ = roc_curve(y_val_lb, y_pred_lb)
            roc_auc = auc(fpr, tpr)
...
def voting_classifier(X_train, y_train, X_val, y_val):
    ...
    print(f"Voting Classifier Accuracy: {voting_clf.score(X_val, y_val)}")

    best_models[voting_clf] = voting_clf.score(X_val, y_val)
    best_params[voting_clf] = best_estimators

    return voting_clf


def save_metrics(y_test, y_pred_final, best_model_key):
...
def build_model():
    try:
        ...
        # search best parameters
        search_best_params(X_train, y_train)
        ...
        # save metrics
        save_metrics(y_test, y_pred_final, best_model_key)
